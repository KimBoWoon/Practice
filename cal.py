...
class Example(tk.Frame):
    ...
    def initUI(self):
        ...
        cls = ttk.Button(self, text="Cls", command=lambda: self.clicked_cls())
        cls.grid(row=1, column=0)
        bck = ttk.Button(self, text="Back", command=lambda: self.clicked_back())
        ...
        div = ttk.Button(self, text="/", command=lambda: self.clicked_btn('/'))
        ...
        mul = ttk.Button(self, text="*", command=lambda: self.clicked_btn('*'))
        ...
        mns = ttk.Button(self, text="-", command=lambda: self.clicked_btn('-'))
        ...
        equ = ttk.Button(self, text="=", command=lambda: self.clicked_equal())
        equ.grid(row=5, column=2)
        pls = ttk.Button(self, text="+", command=lambda: self.clicked_btn('+'))
    ...
    def clicked_equal(self):
        if (self.s == '/' and self.val == '0'):
            self.v.set('0으로 나눌 수 없음')
            self.clicked_cls()
        else:
            self.v.set(eval('{0}{1}{2}'.format(self.temp, self.s, self.val)))
        self.entry.config(textvariable=self.v)
